...
| decentralized | router knows physically-connected neighbors, link costs to neighbors, iterative process of computation, exchange of info with neighbors. "distance vector" algorithms |
...
| aspect | intra-AS | inter-AS |
| --- | --- | --- |
| policy | single admin, so no policy decisions are needed. | admin wants control over how its traffic is routed and who routes through its network. |
| scale | hierarchical routing saves table size, reduces update traffic. | hierarchical routing saves table size, reduces update traffic. |
